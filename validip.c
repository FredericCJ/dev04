...
bool
convertIP2int (char *adresse_ip, unsigned long int *ip_int)
{
  char *IPpattern = "([[:digit:]]{1,3})\\.([[:digit:]]{1,3})\\.([[:digit:]]{1,"
                    "3})\\.([[:digit:]]{1,3})";
  regex_t regex;
  int err;
  regmatch_t pmatch[5];
  ...
  // Compilation expression reguliere et verification 4 subexpressions pour
  // detecter une IP
  ...
  if ((err == 0) &&             // L'expression a ete trouvee (match)
      ...
      (pmatch[0].rm_eo
       == (regoff_t)strlen (adresse_ip)) // Aucun caractere apres le "match"
  )
    {
      ...
      for (int octet_num = 1; octet_num <= 4; octet_num++)
        {
          ...
          char octet_str[4] = { '\0' };
          for (int pos = pmatch[octet_num].rm_so, i = 0;
               pos < pmatch[octet_num].rm_eo; pos++, i++)
            {
